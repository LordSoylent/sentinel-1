...
def is_valid_suncoin_address(address, network='mainnet'):
    ...
    suncoin_version = 140 if network == 'testnet' else 63
    ...
    try:
        ...
        # rescue from exception, not a valid SunCoin address
        return False

    if (address_version != suncoin_version):
        return False
...
# shims 'til we can fix the suncoind side
def SHIM_serialise_for_suncoind(sentinel_hex):
    from models import SUNCOIND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SUNCOIND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in suncoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # suncoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    suncoind_hex = serialise(obj)
    return suncoind_hex


# shims 'til we can fix the suncoind side
def SHIM_deserialise_from_suncoind(suncoind_hex):
    from models import SUNCOIND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(suncoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('suncoin.conf')
