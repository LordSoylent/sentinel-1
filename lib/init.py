...
def has_suncoin_conf():
    import config
    import io

    valid_suncoin_conf = False

    # ensure suncoin_conf exists & readable
    #
    # if not, print a message stating that SunCoin Core must be installed and
    # configured, including JSONRPC access in suncoin.conf
    try:
        f = io.open(config.suncoin_conf)
        valid_suncoin_conf = True
    except IOError as e:
        print(e)

    return valid_suncoin_conf
...
def main():
    ...
    if not has_suncoin_conf():
        print("SunCoinCore must be installed and configured, including JSONRPC access in suncoin.conf")
        sys.exit(1)
