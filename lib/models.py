...
class GovernanceObject(BaseModel):
    ...
    # sync suncoind gobject list with our local relational DB backend
    @classmethod
    def sync(self, suncoind):
        golist = suncoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_suncoind(suncoind, item)
    ...
    @classmethod
    def import_gobject_from_suncoind(self, suncoind, rec):
        import decimal
        import suncoinlib
        import inflection
        ...
        # shim/suncoind conversion
        object_hex = suncoinlib.SHIM_deserialise_from_suncoind(object_hex)
        objects = suncoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from suncoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from suncoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from suncoind - SunCoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(suncoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from suncoind! %s" % e)
            govobj.vote_delete(suncoind)
            return (govobj, None)
    ...
    def vote_delete(self, suncoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(suncoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, suncoind, signal, outcome):
        import suncoinlib
        ...
        output = suncoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = suncoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(suncoind, signal)

    def sync_network_vote(self, suncoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = suncoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SUNCOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 suncoin addr, non-multisig
            if not suncoinlib.is_valid_suncoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid SunCoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (suncoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SunCoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import suncoinlib
        obj_data = suncoinlib.SHIM_serialise_for_suncoind(self.serialise())
    ...
    def prepare(self, suncoind):
        try:
            object_hash = suncoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SUNCOIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not suncoinlib.is_valid_suncoin_address(addr, config.network):
    ...
    def hash(self):
        import suncoinlib
        return suncoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SUNCOIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, suncoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - suncoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, suncoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - suncoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, suncoind):
        now = int(time.time())
        return (self.created_at < (now - suncoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, suncoind):
        if self.is_expired(suncoind):
            return False

        return True

    def is_deletable(self, suncoind):
        if self.is_expired(suncoind):
            return True
