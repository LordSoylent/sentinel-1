...
class SunCoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some suncoind methods
    ...
    @classmethod
    def from_suncoin_conf(self, suncoin_dot_conf):
        from suncoin_config import SunCoinConfig
        config_text = SunCoinConfig.slurp_config_file(suncoin_dot_conf)
        creds = SunCoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = suncoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = suncoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = suncoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_suncoind_proto_version_with_sentinel_ping)
